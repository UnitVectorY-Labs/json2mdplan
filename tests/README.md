...
- `schema.json`
  Optional future JSON Schema for the input document.
- `plan.json`
...
The current `.error` convention is a simple stable key/value text format:

```text
code=missing_field
directive=0
path=department
message=field path "department" does not exist relative to "."
```
